...
def employment_tax(salary: int) -> int:
    ...
    no_eat = month_price - 20             # (예시) 공제 전 가액
    return int(no_eat * 9 / 1000)         # 0.9% 정도로 계산(예시 공식)
...
def work_tax(salary: int) -> int:
    ...
    # 월급(원)을 1,000원 단위 정수로 변환 (예: 3,250,000 → 3250)
    month_thousand = int(str(int(month_price))[:-3] or "0")

    # "이상" ≤ 값 < "미만" 범위에 해당하는 행 찾기
    tax_info = df[(df["이상"] <= month_thousand) & (month_thousand < df["미만"])]

    if tax_info.empty:
        return 0

    # "Unnamed: 2" 컬럼에 세액이 들어있음
    return int(tax_info["Unnamed: 2"].iloc[0])
